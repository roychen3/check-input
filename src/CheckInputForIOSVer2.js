...
const CheckInputForIOS = () => {
  ...
  const [isBeforeInputCapture, setIsBeforeInputCapture] = useState(false);
  ...
  const handleCompositionEnd = (event) => {
    ...
    if (!window.mobileCheck()) {
      const newSelectionStart =
        ...
        (event.data.length - filteredValue.length);
      setSelectionStart(newSelectionStart);
    }
  };

  const handleBeforeInput = (event) => {
    setIsBeforeInputCapture(true);
    console.log(event.data);
    if (event.data.match(re)) {
      event.preventDefault();
      setIsBeforeInputCapture(false);
    }
  };
  ...
  return (
    <div>
      <h1>Check Input Text For IOS Ver2.1</h1>
      <input
        type='text'
        value={value}
